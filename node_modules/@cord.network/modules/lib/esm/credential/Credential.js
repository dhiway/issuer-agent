import { isDidSignature, verifyDidSignature, resolveKey, signatureToJson, signatureFromJson, } from '@cord.network/did';
 ...
 */
export function removeContentProperties(credential, properties) {
    const presentation = 
 ...
 */
export function verifyAgainstSchema(credential, schema) {
 ...
 */
export function fromContent(content, { evidenceIds = [], authorization = null, registry = null } = {}) {
    ...
    const credential = {
        ...
        registry: registry,
        rootHash,
        identifier: Identifier.hashToUri(rootHash, STREAM_IDENTIFIER, STREAM_PREFIX),
    };
    verifyDataStructure(credential);
    return credential;
}
 ...
 */
export async function verifyCredential(credential, { schema } = {}) {
    verifyDataStructure(credential);
 ...
 */
export async function verifyPresentation(presentation, { schema, challenge, didResolveKey = resolveKey } = {}) {
...
export function isPresentation(input) {
    return (isICredential(input) &&
        isDidSignature(input.holderSignature));
}
/**
