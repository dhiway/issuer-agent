...
import { SDKErrors, Identifier, Crypto, DataUtils, jsonabc, } from '@cord.network/utils';
...
function jsonLDcontents(content, expanded = true) {
    ...
    if (!schemaId)
        new SDKErrors.SchemaIdentifierMissingError();
 ...
 */
export function toJsonLD(content, expanded = true) {
 ...
 */
export function hashContents(content, options = {}) {
    // apply defaults
    ...
    processed.forEach(({ digest, nonce, statement }) => {
        ...
        if (!nonce)
            throw new SDKErrors.ContentNonceMapMalformedError(statement);
 ...
 * @param options.hasher The hasher to be used. Required but defaults to 256 bit blake2 over `${nonce}${statement}`.
 */
export function verifyDisclosedAttributes(content, proof, options = {}) {
    // apply defaults
    const defaults = { canonicalisation: makeStatementsJsonLD };
    const canonicalisation = options.canonicalisation || defaults.canonicalisation;
    const { nonces } = proof;
    ...
    if (verified !== true) {
        throw new SDKErrors.ContentUnverifiableError('One or more statements in the content could not be verified', { cause: errors });
    }
}
/**
 ...
 */
export function verify(inputContent, schema) {
 ...
 */
export function fromNestedSchemaAndContent(schema, nestedSchemas, contents, holder, issuer) {
 ...
 */
export function fromSchemaAndContent(schema, contents, holder, issuer) {
 ...
 */
export function isIContent(input) {
    try {
