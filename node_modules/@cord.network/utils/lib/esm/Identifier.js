...
import { SCHEMA_PREFIX, REGISTRY_PREFIX, STREAM_PREFIX, SCORE_PREFIX, } from '@cord.network/types';
import { ACCOUNT_IDENTIFIER_PREFIX, REGISTRY_IDENT, SCHEMA_IDENTIFIER, STREAM_IDENTIFIER, SCORE_IDENTIFIER, } from '@cord.network/types';
const defaults = {
...
function encodeIdentifier(key, iDPrefix) {
    ...
    assert(iDPrefix >= 0 && iDPrefix <= 16383 && ![46, 47].includes(iDPrefix), 'Out of range IdentifierFormat specified');
    ...
    const input = u8aConcat(iDPrefix < 64
        ? [iDPrefix]
        : [
            ((iDPrefix & 0b0000_0000_1111_1100) >> 2) | 0b0100_0000,
            (iDPrefix >> 8) | ((iDPrefix & 0b0000_0000_0000_0011) << 6),
        ], u8a);
    return base58Encode(u8aConcat(input, pphash(input).subarray(0, [32, 33].includes(u8a.length) ? 2 : 1)));
}
export function hashToIdentifier(identifier, iDPrefix) {
    assert(identifier, 'Invalid key string passed');
    const id = encodeIdentifier(identifier, iDPrefix);
    return id;
}
export function hashToUri(identifier, iDPrefix, prefix) {
    assert(identifier, 'Invalid key string passed');
    const id = encodeIdentifier(identifier, iDPrefix);
    return `${prefix}${id}`;
}
export function uriToIdentifier(identifier) {
    assert(identifier, 'Invalid key string passed');
 ...
 */
export function checkIdentifier(address) {
    let decoded;
    try {
 ...
 */
export function getAccountIdentifierFromAddress(address) {
 ...
 */
export function getAccountAddressFromIdentifier(address) {
